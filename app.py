...
# --- 2. 側邊欄：持股管理 (含記憶邏輯) ---
if 'my_stocks_list' not in st.session_state:
    st.session_state.my_stocks_list = "2330, 3017, 3450, NVDA"

st.sidebar.header("👤 個人持股管理")
my_input = st.sidebar.text_input("輸入股票代碼 (逗號隔開)", st.session_state.my_stocks_list)
st.session_state.my_stocks_list = my_input
...
WATCHLIST = {
    ...
    "NVIDIA GB200 供應鏈": ["2317.TW", "2308.TW", "2301.TW", "3013.TW", "6213.TW", "3376.TW"],
...
# 初始化全局選擇標的
if 'global_selected_stock' not in st.session_state:
    st.session_state.global_selected_stock = "2330.TW"
...
for i, cat in enumerate(WATCHLIST.keys()):
    with tabs[i]:
        ...
        if not df_summary.empty:
            ...
            if sel:
                st.session_state.global_selected_stock = sel
        else:
            st.warning(f"{cat} 暫無資料")

# --- 5. 詳細分析區 (使用全局選擇標的) ---
target = st.session_state.global_selected_stock
st.divider()
