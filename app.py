...
STOCK_MAP = {
    ...
    "00679B.TW": "元大美債20年", "00929.TW": "復華台灣科技優息", "2409.TW": "友達",
    "2312.TW": "金寶", "2520.TW": "冠德", "2609.TW": "陽明", "2634.TW": "漢翔",
    "2890.TW": "永豐金", "3051.TW": "鴻名", "3062.TW": "建漢", "4906.TW": "正文",
...
def analyze_stock(ticker):
    try:
        ...
        # 財報面 (毛利率) - 僅限公司標的，跳過 ETF
        margin = 0
        if "00" not in ticker:
            try:
                q_fin = stock.quarterly_income_stmt.T
                if q_fin.empty: q_fin = stock.quarterly_financials.T
        ...
        if "Rate Limit" in str(e): return "LIMIT"
        return None
...
default_stocks = query_params.get("stocks", "2330, 3017, 3450, NVDA, 00679B")
...
for s in my_input.split(","):
    s = s.strip().upper()
    if (s.isdigit() or "00" in s) and ".TW" not in s: s = f"{s}.TW"
    processed_stocks.append(s)

# 最新產品線分類 (GB200 Blackwell)
WATCHLIST = {
...
for i, cat in enumerate(WATCHLIST.keys()):
    with tabs[i]:
        ...
        for t in WATCHLIST[cat]:
            res = analyze_stock(t)
            if res == "LIMIT":
                st.error("⚠️ Yahoo Finance 查詢太頻繁，請等 1 分鐘後再重新整理。")
                break
        ...
        if not df_all.empty:
            ...
            if "00" not in target:
                try:
                    ...
                    if q_fin.empty: q_fin = s_obj.quarterly_financials.T
                    if not q_fin.empty:
                        c1, c2 = st.columns(2)
                        with c1:
                            st.subheader("📊 獲利能力")
                            ...
                            eps_col = [c for c in q_fin.columns if 'EPS' in c]
                            if eps_col: p_df['EPS'] = q_fin[eps_col[0]].round(2)
                ...
                except: st.info("此標的未提供詳細季度報表。")
        else:
            st.error("無法載入 K 線資料。")
    except Exception as e:
        if "Rate Limit" in str(e): st.error("⚠️ 流量限制中，請稍候再試。")
